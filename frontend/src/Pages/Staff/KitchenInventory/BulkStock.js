...
>>>>>>> e031a7972c9fb6863d2007e5dd22afd48347f07a
  ...
  useEffect(() => {
    ...
    const uniqueCategories = [
      ...new Set(BStockList.map((BulkStock) => BulkStock.category)),
    ];
  ...
  BStockList.forEach((BulkStock) => {
    ...
    if (!aggregatedStockList[key]) {
      ...
      if (
        new Date(aggregatedStockList[key].createdAt) <
        new Date(BulkStock.createdAt)
      ) {
...
>>>>>>> e031a7972c9fb6863d2007e5dd22afd48347f07a
  ...
  //search and filter
  const filteredStockList = Object.values(aggregatedStockList).filter(
    (BulkStock) =>
      BulkStock.bname.toLowerCase().includes(searchkey.toLowerCase()) &&
      (!filterCategory ||
        BulkStock.bcategory.toLowerCase() === filterCategory.toLowerCase())
  );
  // Sort data function
  const sortData = () => {
          ...
          {sortData(filteredStockList).map((bstock) => (
            <View style={styles.row} key={bstock._id}>
              ...
              <Text style={styles.cell}>
                {new Date(bstock.createdAt).toLocaleString()}
              </Text>
              <Text style={styles.cell}>
                {new Date(bstock.updatedAt).toLocaleString()}
              </Text>
            </View>
          ))}
        </View>
          ...
          <div className="row justify-content-center mb-3">
            ...
            <div className="col-auto">
              ...
              >
                ...
                {[
                  ...new Set(
                    filteredStockList.map((BulkStock) => BulkStock.bcategory)
                  ),
                ].map((bcategory) => (
                  <option key={bcategory} value={bcategory}>
                    {bcategory}
                  </option>
                ))}
              </select>
            ...
            <div className="col-auto">
              ...
              >
                ...
                <option value="newestCreated">
                  Newest to Oldest (Created)
                </option>
                <option value="oldestCreated">
                  Oldest to Newest (Created)
                </option>
                <option value="newestUpdated">
                  Newest to Oldest (Updated)
                </option>
                <option value="oldestUpdated">
                    ...
                    <tr>
                      <td>
                        {nameToUpdate === BulkStock._id ? (
                          ...
                          <td>
                            <Link
                              to={`/kitchenBulkStock/${BulkStock.bname}`}
                              className="invisible-link-button"
                            >
                              {BulkStock.bname}
                            </Link>
                          </td>
